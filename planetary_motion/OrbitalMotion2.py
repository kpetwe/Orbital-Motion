...
def sunPos(d = 0.0, ct = "rect", csys = "eclip"):    
    ...
    if (csys == "eclip" and ct == "rect"):
        return x, y, z, oblecl
    elif (csys == "eclip" and ct == "sphere"):
        long, lat, r = Coordinates.sphere(x, y, z)
        return long, lat, r, oblecl

    # Rotate coordinates using calculated oblecl
    xeq, yeq, zeq = Coordinates.ecEq(x, y, z, oblecl)

    if (csys == "equat" and ct == "rect"):
        return xeq, yeq, zeq, oblecl

    # Convert to RA and Decl
    RA, Decl, r = Coordinates.sphere(xeq, yeq, zeq)

    return RA, Decl, r, oblecl
...
def getPlanPosGeo(n, d, ct = "rect", csys = "equat"):
    ...
    if (ct == "rect" and csys == "eclip"):
        return plan_geo
    elif (ct == "sphere" and csys == "eclip"):
        return Coordinates.sphere(plan_geo[0], plan_geo[1], plan_geo[2])

    plan_geo_eq = Coordinates.ecEq(plan_geo[0], plan_geo[1], plan_geo[2], sun_arr[3])

    if (ct == "sphere" and csys == "equat"):
        return Coordinates.sphere(plan_geo_eq[0], plan_geo_eq[1], plan_geo_eq[2])
